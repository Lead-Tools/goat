...
Функция СоздатьСценарий(Модуль, Переменные, ИмяПроцедурыОбработчикаОшибок, Отладка = Ложь) Экспорт
	...
	Сценарий.Вставить("Переменные", Переменные);
	Сценарий.Вставить("ОбработчикОшибок", ОбработчикОшибок(ИмяПроцедурыОбработчикаОшибок, Модуль));
	Если Отладка Тогда
...
Процедура НачатьВыполнение(Сценарий, СледующийОбработчикЗадания = Неопределено) Экспорт
		
	КонвейерЗаданийСлужебныйКлиент.ПодготовитьСценарий(
		Сценарий,
		Сценарий.ОбработчикОшибок,
...
Функция ДиалогВыбораФайла(Сценарий, ИмяПроцедурыОбработчика, ДиалогВыбораФайла, ИмяПроцедурыОбработчикаОшибок = Неопределено) Экспорт
	
	Обработчик = Новый ОписаниеОповещения(ИмяПроцедурыОбработчика, Сценарий.Модуль, Сценарий.Переменные);
	
	ДекорированныйОбработчик = КонвейерЗаданийСлужебныйКлиент.ДекорироватьОбработчик(Обработчик);
	
	ОбработчикОшибок = Сценарий.ОбработчикОшибок;
	Если ИмяПроцедурыОбработчикаОшибок <> Неопределено Тогда
		ОбработчикОшибок = ОбработчикОшибок(ИмяПроцедурыОбработчикаОшибок, Сценарий.Модуль);
	КонецЕсли;
...
Функция СозданиеКаталога(Сценарий, ИмяКаталога, ИмяПроцедурыОбработчикаОшибок = Неопределено) Экспорт
	
	Обработчик = Неопределено; // пока непонятно есть ли смысл давать пользователю возможность указывать обработчик в данном задании
...
Функция УдалениеФайлов(Сценарий, Путь, Маска = Неопределено, ИмяПроцедурыОбработчикаОшибок = Неопределено) Экспорт
...
Функция ДиалогВопроса(Сценарий, ИмяПроцедурыОбработчика, ТекстВопроса, Кнопки, Таймаут = 0, КнопкаПоУмолчанию = Неопределено, Заголовок = Неопределено, КнопкаТаймаута = Неопределено) Экспорт
...
Функция ОбработчикОшибок(ИмяПроцедуры, Модуль)

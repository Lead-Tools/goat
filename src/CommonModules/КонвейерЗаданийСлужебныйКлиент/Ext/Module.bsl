...
Функция ПостроитьКонвейернуюЛинию(Задания, ОбработчикОшибок, СледующийОбработчикЗадания) Экспорт
	
	ИндексЗадания = Задания.Количество();
	
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("СледующийОбработчикЗадания", СледующийОбработчикЗадания);
	...
	ОбработчикЗадания = ОбработчикЗадания(_ВыполнитьЗаданиеОстановкаКонвейера, Неопределено, ОбработчикОшибок);
	
	СтекОкончанийУсловныхБлоков = Новый Массив;
	СтекОкончанийПопыток = Новый Массив;
	...
	Пока ИндексЗадания > 0 Цикл
		...
		Если СтекТочекВходаПослеИсключения.Количество() > 0 Тогда // взять с вершины стека
			ПараметрыЗадания.ОбработчикЗаданияНачалоИсключения = СтекТочекВходаПослеИсключения[СтекТочекВходаПослеИсключения.ВГраница()]; 
		КонецЕсли; 
		...
		Если Задание.Модуль = ЭтотОбъект Тогда
			...
			ИначеЕсли Задание.ИмяПроцедуры = "_ВыполнитьЗаданиеНачалоУсловногоБлока" Тогда
				...
				ПараметрыЗадания.ОбработчикЗаданияОкончаниеУсловногоБлока = СнятьПоследнееЗначение(СтекОкончанийУсловныхБлоков);
				
			ИначеЕсли Задание.ИмяПроцедуры = "_ВыполнитьЗаданиеОкончаниеПопытки" Тогда
				
				// добавить в стек окончаний
				
				СтекОкончанийПопыток.Добавить(ОбработчикЗадания);
				
			ИначеЕсли Задание.ИмяПроцедуры = "_ВыполнитьЗаданиеНачалоИсключения" Тогда
				
				ОбработчикЗаданияОкончаниеПопытки = СнятьПоследнееЗначение(СтекОкончанийПопыток);
				
				СтекТочекВходаПослеИсключения.Добавить(ОбработчикЗадания.ДополнительныеПараметры.СледующийОбработчикЗадания);
				ОбработчикЗадания.ДополнительныеПараметры.СледующийОбработчикЗадания = ОбработчикЗаданияОкончаниеПопытки;
				
				// снять со стека окончаний и воткнуть следующий обработчик ОбработчикЗадания(Задание, <>, ОбработчикОшибок)
				// добавить в стек исключений
				
				// далее каждому заданию втыкать ОбработчикИсключения
				
			ИначеЕсли Задание.ИмяПроцедуры = "_ВыполнитьЗаданиеНачалоПопытки" Тогда
...
Процедура _ВыполнитьОбработчикЗадания(КонтекстЗадания, ПараметрыОбработчикаЗадания) Экспорт
	...
	Исключение
		...
		Если СтандартнаяОбработка = Истина Тогда // безопасная проверка
			...
			Если ПараметрыЗадания.ОбработчикЗаданияНачалоИсключения = Неопределено Тогда
				ВызватьИсключение;
			Иначе
				Вызвать(ПараметрыЗадания.ОбработчикЗаданияНачалоИсключения, КонтекстЗадания.ОбщиеПараметры, "ПрикладноеИсключение!");
			КонецЕсли; 
		КонецЕсли;
...
Процедура _ОбработатьОшибку(ИнформацияОбОшибке, СтандартнаяОбработка, ПараметрыДекоратора) Экспорт
	...
	Если СтандартнаяОбработка = Истина Тогда // безопасная проверка
		Если ПараметрыДекоратора.ОбработчикЗаданияНачалоИсключения <> Неопределено Тогда
			Вызвать(ПараметрыДекоратора.ОбработчикЗаданияНачалоИсключения, ПараметрыДекоратора.ОбщиеПараметры, "СистемноеИсключение!");
...
Процедура _ВыполнитьЗаданиеНачалоУсловногоБлока(КонтекстЗадания, ПараметрыЗадания) Экспорт
	...
	Если РезультатУсловия <> Истина Тогда
		...
		Вызвать(
			ПараметрыЗадания.ОбработчикЗаданияОкончаниеУсловногоБлока,
...
Процедура _ВыполнитьЗаданиеНачалоПопытки(КонтекстЗадания, ПараметрыЗадания) Экспорт
	...
	Вызвать(
		КонтекстЗадания.СледующийОбработчикЗадания,
		КонтекстЗадания.ОбщиеПараметры,
		ИмяМодуля() + "._ВыполнитьЗаданиеНачалоПопытки()"
	);	
	
КонецПроцедуры

Процедура _ВыполнитьЗаданиеНачалоИсключения(КонтекстЗадания, ПараметрыЗадания) Экспорт
	...
	Вызвать(
		КонтекстЗадания.СледующийОбработчикЗадания,
		КонтекстЗадания.ОбщиеПараметры,
		ИмяМодуля() + "._ВыполнитьЗаданиеНачалоИсключения()"
	);	
	
КонецПроцедуры

Процедура _ВыполнитьЗаданиеОкончаниеПопытки(КонтекстЗадания, ПараметрыЗадания) Экспорт
	...
	Вызвать(
		КонтекстЗадания.СледующийОбработчикЗадания,
		КонтекстЗадания.ОбщиеПараметры,
		ИмяМодуля() + "._ВыполнитьЗаданиеОкончаниеПопытки()"
	);	
...
Процедура _ВыполнитьЗаданиеОстановкаКонвейера(КонтекстЗадания, ПараметрыЗадания) Экспорт
	
	Вызвать(
		ПараметрыЗадания.СледующийОбработчикЗадания,
		КонтекстЗадания.ОбщиеПараметры,
		ИмяМодуля() + "._ВыполнитьЗаданиеОстановкаКонвейера()"
	);
...
Функция ДекорироватьОбработчик(Знач Обработчик, ИмяДекоратора = "_ВыполнитьОбработчик2") Экспорт
	...
	ПараметрыДекоратора.Вставить("ОбработчикЗаданияНачалоИсключения", Неопределено);
...
Процедура ПодготовитьПараметрыДекоратора(КонтекстЗадания, ПараметрыЗадания)
	...
	ПараметрыДекоратора.ОбработчикЗаданияНачалоИсключения = ПараметрыЗадания.ОбработчикЗаданияНачалоИсключения;

...
Процедура ТестАсинхронный(Команда)
	...
	Сценарий = Коза.СоздатьСценарий(ЭтотОбъект, Переменные, "ОбработчикОшибок");
	
	#Область Коза	
	
	Коза.ДиалогВыбораФайла(Сценарий, "ОбработатьВыборПапки", ДиалогВыбораПапки);
	Коза.Оператор_Если(Сценарий, "ПроверкаУсловия");
		Коза.Оператор_Попытка(Сценарий);
			Коза.СозданиеКаталога(Сценарий, СсылкаНаИмяПапки);
			Коза.СозданиеКаталога(Сценарий, СсылкаНаИмяПодпапки);
			Коза.УдалениеФайлов(Сценарий, СсылкаНаИмяПодпапки);
